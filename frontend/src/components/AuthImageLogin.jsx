...
const AuthImageLogin = ({ title, subtitle }) => {
  ...
  const getHexagonSize = (size) => {
    switch (size) {
      case "small":
        return { container: "w-16 h-16", icon: "w-6 h-6" };
      case "medium":
        return { container: "w-20 h-20", icon: "w-8 h-8" };
      case "large":
        return { container: "w-24 h-24", icon: "w-10 h-10" };
      default:
        return { container: "w-20 h-20", icon: "w-8 h-8" };
    }
  };
  ...
  return (
    <section className="hidden  lg:flex flex-col bg-pink-700 items-center justify-center p-12 relative overflow-hidden">
      ...
      <div className="absolute  inset-0">
        ...
        ></div>
        <div
          className="absolute top-1/2 left-1/2 w-20 h-20 bg-pink-300/15 rounded-full blur-xl transform -translate-x-1/2 -translate-y-1/2"
      ...
      {/* Hexagonal elements */}
      <div className="relative top-50" style={{ zIndex: 2 }}>
        {hexagonElements.map(({ Icon, size, position, delay }, i) => {
          ...
          return (
            ...
            >
              <Icon
                className={`${icon} ${
                  position === "center" ? "text-white" : "text-pink-100"
                }`}
                strokeWidth={1.5}
              />
            </div>
      ...
      {/* Main content */}
      <div className="relative z-10 text-center mt-92">
        <h2 className="text-4xl font-bold text-white righteous-regular mb-4 leading-tight">
          {title}
        </h2>
        <p className="text-pink-100 text-lg righteous-regular leading-relaxed max-w-sm mx-auto mb-8">
          {subtitle}
        </p>
      </div>
